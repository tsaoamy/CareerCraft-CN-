...
export class TalentRepository {
  ...
  static async upsert(
    ...
    if (existing) {
      execute(
        ...
        WHERE user_id = ?`,
        [
          JSON.stringify(analysis.skill_structure),
          analysis.career_direction,
          JSON.stringify(analysis.career_path),
          analysis.job_match_score,
          JSON.stringify(analysis.capability_tags),
          analysis.growth_potential,
          analysis.career_risk_score,
          JSON.stringify(analysis.career_risk_factors),
    ...
    execute(
      `INSERT INTO talent_profiles (
        id, user_id, skill_structure, career_direction, career_path,
        job_match_score, capability_tags, growth_potential,
        career_risk_score, career_risk_factors,
  ...
  static async listAll(page = 1, pageSize = 20): Promise<{
    ...
    const profiles = queryAll(
      `SELECT tp.*, u.username, u.email, u.nickname
  ...
  static async getSkillDistribution(): Promise<Record<string, unknown>[]> {
    ...
    const profiles = queryAll(
      "SELECT capability_tags FROM talent_profiles WHERE capability_tags != '[]'"
    );

    const tagCounts: Record<string, number> = {};
    for (const p of profiles) {
      try {
        const tags = JSON.parse(p.capability_tags as string) as string[];
